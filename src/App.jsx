...
const App = () => {
  ...
  const [iconTopType, setIconTopType] = useState('none'); // Üst Simge
  const [iconBottomType, setIconBottomType] = useState('none'); // Alt Simge
  ...
  const [textScaleMain, setTextScaleMain] = useState(100); // Ana Yazı Boyutu (%)
  const [textScaleMid, setTextScaleMid] = useState(100); // Orta Yazı Boyutu (%)
  const [textScaleSub, setTextScaleSub] = useState(100); // Alt Yazı Boyutu (%)
  const [iconScale, setIconScale] = useState(100); // Simge Boyutu (%)
  const [hasDivider, setHasDivider] = useState(false); // Ayraç Çizgisi
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto flex flex-col md:flex-row gap-6 md:gap-12 px-4 sm:px-6 relative">
        ...
        <div className="w-full md:w-auto flex flex-col gap-6 items-center md:items-start z-10 shrink-0">
          ...
          <SettingsCard 
            ...
            setTextScaleMid={setTextScaleMid}
            textScaleSub={textScaleSub}
            setTextScaleSub={setTextScaleSub}
            hasDivider={hasDivider}
            setHasDivider={setHasDivider}
